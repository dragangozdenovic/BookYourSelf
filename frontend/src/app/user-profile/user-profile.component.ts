...
export class UserProfileComponent implements OnInit {
  ...
  constructor(private service: AppService, private http: HttpClient,private roomService : RoomService,private hotelSerice : HotelService,
              private avioService:AvioCompanyService,private rentaService:RentaCarService) {}
  ...
  changeHotelInfo(event) {
    event.preventDefault();
    this.userProfile.hotel.name = event.target.querySelector("#nameH").value;
    this.userProfile.hotel.address = event.target.querySelector("#address").value;
    this.userProfile.hotel.promoDescription = event.target.querySelector("#promo").value;

    this.service.changeHotel(this.userProfile.hotel);
  }
  ...
  showFavor() {
    ...
    this.http.get('api/hotel/all').subscribe(data => {
      this.HotelFavour2 = data;
      this.HotelFavour1 = data;
      this.HotelFavour = data;
      this.HotelFavour = this.HotelFavour1.filter((HotelFavour) => HotelFavour.id == this.userProfile.hotel.id);
      this.HotelFavourOne = this.HotelFavour[0].priceList;
    });
  }

  showRooms() {
    this.http.get('api/room/all').subscribe(data => {
      localStorage.setItem('room', JSON.stringify(data))
    });
    this.RoomHotel2 = JSON.parse(localStorage.getItem('room'));
    this.RoomHotell = this.RoomHotel2;
    this.RoomHotel = this.RoomHotel2.filter((RoomHotel) => RoomHotel.hot == this.userProfile.hotel.id);
    this.RoomHotell = this.RoomHotel2.filter((RoomHotel1) => RoomHotel1.hot == this.userProfile.hotel.id && RoomHotel1.reserved == false);
    console.log(this.RoomHotell);
  }

  editRoom(event) {
    event.preventDefault();
    var name = event.target.querySelector("#name" + this.editRoomId.id).value;
    var dateI = event.target.querySelector("#dateI" + this.editRoomId.id).value;
    var dateO = event.target.querySelector("#out" + this.editRoomId.id).value;
    var price = event.target.querySelector("#price" + this.editRoomId.id).value;
    var bed = event.target.querySelector("#bed" + this.editRoomId.id).value;
    var floor = event.target.querySelector("#floor" + this.editRoomId.id).value;

    this.roomService.editRoom(this.editRoomId.id, name, dateI, dateO, price, this.editRoomId.number_of_room, this.editRoomId.reserved, bed, floor);
  ...
  DeleteRoom(room) {
    this.deleteRoom = this.RoomHotel;
    this.deleteRoom1 = this.RoomHotel;

    this.deleteRoom = this.deleteRoom1.filter((deleteRoom) => deleteRoom.id == room);

    this.roomService.deleteRoom(this.deleteRoom[0]);
  }

  addRoom(event) {
    event.preventDefault();
    var name = event.target.querySelector("#na").value;
    var dateI = event.target.querySelector("#dateI").value;
    var dateO = event.target.querySelector("#out").value;
    var price = event.target.querySelector("#priceR").value;
    var number_room = event.target.querySelector("#number").value;
    var number_of_bed = event.target.querySelector("#bed").value;
    var number_of_floor = event.target.querySelector("#floor").value;

    this.RoomHotell = this.RoomHotel2.filter((RoomHotel) => RoomHotel.number_of_room == number_room && RoomHotel.hot == this.userProfile.hotel.id);
    if (this.RoomHotell.length > 0) {
      alert("Postoji soba sa tim imenom");
    } else
      this.roomService.addR(name, dateI, dateO, price, number_room, false, this.userProfile.hotel.id, number_of_bed, number_of_floor);
  ...
  createHotels(event) {
    event.preventDefault();
    var name = event.target.querySelector("#hotNa").value;
    var address = event.target.querySelector("#hotAddr").value;
    var promo = event.target.querySelector("#hotPromo").value;
    this.hotelSerice.addHotel(name, address, promo);
  }

  createRentaa(event) {
    event.preventDefault();
    var name = event.target.querySelector("#rentNa").value;
    var address = event.target.querySelector("#rentAddr").value;
    var promo = event.target.querySelector("#rentPromo").value;
    this.rentaService.addRenta(name, address, promo);
  }

  allHotel() {
    return this.http.get('api/aviocompanies/all').subscribe(data => this.idAvio = data);
  }

  allRenta() {
    return this.http.get('api/hotel/all').subscribe(data => this.idHotel = data);
  }
